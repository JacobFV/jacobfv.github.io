...
$$\tau_{narrative} A_{\text{self}}(t) = g(\{A_{\text{self}}(t)\}_{t<t})$$ — the compression of memories into coherent trajectories. There is metacognition $$\tau_{metacognition} I(A_{\text{self}}^{\text{model}}, A_{\text{self}}^{\text{actual}}) > 0$$
