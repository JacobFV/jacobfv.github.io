...
These are ideological attractors—fixed points in idea-space, not because of doctrine or revelation, but because of geometry. Once consciousness becomes reflexive, certain recognitions become unavoidable.[^schizo] There is the recognition of otherness: the persistent boundary between self and non-self. There is agency: the realization that one’s internal state can alter future states of the system. There is impermanence: the corresponding realization that an agent's impact to the world can be washed out by entropy. There is mortality: the realization that one's own existential condition is impermenent and all meaning it has generated will eventually dissapate. There is time and narrative: the compression of memories into coherent trajectories. There is normativity: the emergence of value-gradients over possible states. There is coordination: the recognition that some structures are sustained not in isolation but through mutual correlation with other agents. And these are just some of infinitely many tautological invariants that can be derived from the existential condition.
